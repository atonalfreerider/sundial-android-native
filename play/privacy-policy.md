...
**Calendar events.** If you grant calendar permission and choose calendars to display, Sundial reads those events (titles, times and calendar colours) from Android's calendar storage to draw them on its dials. Calendar access is read-only. Calendar data is processed only on your device and is never transmitted, stored on our servers, or shared.

**Birth date, birth time and sun sign.** If you use the optional astrology mode, the birth details you enter are saved in the app's private storage on your device. They are used only on your device to draw the zodiac instrument and to write your daily horoscope.
...
- **Read calendar** — to show the calendars you choose on the dials (optional).
- **Set wallpaper** — for the optional celestial wallpaper.
- **Internet** — only to send a report you choose to submit (Google ML Kit also uses it for the diagnostics above).
- Background work and boot permissions — to refresh the optional wallpaper every 15 minutes.
...
Sundial is not directed to children under 13, and we do not knowingly collect information from children.
