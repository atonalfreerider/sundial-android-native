...
class AstrologyPanel(context: Context) : LinearLayout(context) {
    ...
    private val reportAction = controls.action("REPORT THIS READING", "Report today's AI-written horoscope") {
    ...
    init {
        ...
        addView(reportAction)
        addView(status)
        addView(controls.label("Readings are written by on-device AI for entertainment only.", 11f, 0x77FFFFFF).apply {
            setPadding(controls.dp(4), 0, controls.dp(4), controls.dp(4))
        })
        setHasReading(false)
    ...
    fun setHasReading(value: Boolean) {
        reportAction.isEnabled = value
        reportAction.alpha = if (value) 1f else .46f
    }
